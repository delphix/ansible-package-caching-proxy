...
@test "The vhost for the docker registry should be available" {
    run curl -sSq -H 'Host: registry2' http://localhost/v2/
    [ "$status" -eq 0 ]
    [[ "$output" =~ "{}" ]]
}

@test "The docker registry's /v2/ url should return valid JSON" {
    run bash -c "curl -sSq -H 'Host: registry2' http://localhost/v2/ | python -m json.tool"
    [ "$status" -eq 0 ]
}
...
EOF
    run docker build \
        --tag=registry2.yourdomain.local/test-ansible-package-caching-proxy \
        /tmp/docker-build
    [ "$status" -eq 0 ]
    run docker push registry2.yourdomain.local/test-ansible-package-caching-proxy
    [ "$status" -eq 0 ]
}
